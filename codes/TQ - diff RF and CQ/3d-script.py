...
prevTAU = tau[0]
baseline = open("./params/baseline.txt", "r").read().split(",")
...
def runSimulations(index1, index2, indexTau, prevT1, prevT2):     
  for i in range(len(tau)):
    ...
    first_points = [abs(np.loadtxt(filename,usecols=(1))[0])/Integral0 for filename in filenames]
    ...
    df.to_csv("TQE"+str(tau[indexTau])+".csv", sep=' ', header=False, float_format='%.3f', index=False)
    ...
    maximal_sig = np.max(intensities)
    plt.pcolor(intensities, cmap="RdPu", vmin=0, vmax=maximal_sig)
    plt.xticks(x_positions, x_labels)
    plt.yticks(y_positions, y_labels)
    plt.colorbar()
    plt.title("Intensity of signal VS Pulse durations")
    plt.xlabel("Duration of each of the last 3 pulses")
    plt.ylabel("Duration of the first pulse")
    plt.savefig("TQ"+str(tau[indexTau])+".png")
    os.system("rm *.fid")   
    indexTau = indexTau + 1
